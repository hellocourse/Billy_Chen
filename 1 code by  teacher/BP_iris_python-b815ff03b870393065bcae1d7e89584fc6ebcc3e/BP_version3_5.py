...
class BPNeuralNetwork:
       ...
       def BPalgorithm(self,X,y,step=0.3,times=50):
              ...
              for n in range(times):
                     ...
                     print("目前误差:current",self.error)
              print("训练集正确率：",float(self.d)/(times*len(X)))
              print("均方误差：",float(self.error)/(times*len(X)))
       ...
       def generalize(self,X,y):
              ...
              print(self.d)
              print("测试集正确率：",float(self.d)/len(X))
              print("均方误差：",float(self.error)/len(X))
