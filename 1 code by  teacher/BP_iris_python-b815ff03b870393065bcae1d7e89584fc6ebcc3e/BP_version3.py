...
class BPNeuralNetwork:
       ...
       def BPalgorithm(self,X,y,step=0.3,times=50):
              ...
              for n in range(times):
                     ...
                     # 训练集正确率：
                     # 均方误差：
              # print("training_accuracy",float(self.d)/(times*len(X)))
              # print("training_error",float(self.error)/(times*len(X)))
       ...
       def generalize(self,X,y):
              ...
              for s in range(len(X)):
                     ...
                     X_s=np.hstack((X_s,[1]))
                     y_s=y[s]
                     self.Forward(X_s,y_s)
                     #测试集正确率：
                     #均方误差：
              print("correct_numbers",self.d)  #正确个数
              print("test_accuracy",float(self.d)/len(X))
              # print("test_error",float(self.error)/len(X))
